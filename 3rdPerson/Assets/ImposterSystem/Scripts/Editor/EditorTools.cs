...
namespace ImposterSystem{
	...
	internal static class EditorTools {
...
#endif
            if (
                (version[0] == '5' && version[1] == '.' && int.Parse(version[2].ToString()) >= 6) ||
                version[0] == '2')
            {
                Debug.LogError("IMPORTANT!!! READ THIS!!!\nYou must manually disable 'allowHDR' on 'imposterCamera' gameobject. Click HERE to focus on it.", ImpostersHandler.Instance.getImposterCamera());
            }
        }
		...
		public static void SetUpImposterController(){
            ...
            bool importFromLODGroup = hasLodGroup ? EditorUtility.DisplayDialog("Setup BillbiardController", "Are you wanna replace LODGroup with ImposterController?", "Yes", "No") : false;
            bool setAllRenderersToLODs = importFromLODGroup ? false : EditorUtility.DisplayDialog("Setup BillbiardController", "Are you wanna set all child renderers to Imposter LODs?", "Yes", "No");
            Renderer[] _renders;
