...
namespace ImposterSystem
{
    ...
    internal class ImposterControllerEditor : Editor
    {
        private class LODAction
        {
            ...
            public LODAction(List<LODGroupGUI.LODInfo> lods, float percentage, Vector2 clickedPosition, SerializedProperty propLODs, ImposterControllerEditor.LODAction.Callback callback)
            {
                this.m_LODs = lods;
            ...
            public void InsertLOD()
            {
                if (this.m_LODsProperty.isArray)
                {
                    ...
                    arrayElementAtIndex.FindPropertyRelative("screenRelativeTransitionHeight").floatValue = this.m_Percentage;
        ...
        private void RenderForSceneCamera(Camera camera)
        {
            ...
            imposterPreviewGO.GetComponent<MeshFilter>().sharedMesh = Helper.NewPlane(Vector3.zero, Vector3.one * m_ImposterController.quadSize / 2, new Vector4(0, 0, 1, 1));
            ...
            int texRes = ImposterLODUtility.GetImposterTextureResolution(screenSize, (int)lod.minImposterResolution, (int)lod.maxImposterResolution);
            ...
            imposterPreviewGO.transform.position = m_ImposterController.bounds.center + direction.normalized * m_ImposterController.ZOffset * m_ImposterController.quadSize;
        ...
        public override void OnInspectorGUI()
        {
            ...
            updateBehavior.enumValueIndex = (int)(ImposterOrientation)EditorGUILayout.EnumPopup("Update Behavior", (ImposterController.UpdateBehavior)updateBehavior.enumValueIndex);
            errorCameraAngle.floatValue = EditorGUILayout.Slider("Error camera angle", errorCameraAngle.floatValue, 0.1f, 180);
            ...
            if (useErrorLightAngle.boolValue)
                errorLightAngle.floatValue = EditorGUILayout.Slider("Error light angle", errorLightAngle.floatValue, 0.1f, 180);
            ...
            List<LODGroupGUI.LODInfo> list = LODGroupGUI.CreateLODInfos(this.m_NumberOfLODs, rect, (int i) => string.Format("LOD {0}", i), (int i) => base.serializedObject.FindProperty(string.Format("m_LODs.Array.data[{0}].screenRelativeTransitionHeight", i)).floatValue);
            ...
            if (this.m_NumberOfLODs > 0 && this.activeLOD >= 0 && this.activeLOD < this.m_NumberOfLODs)
            {
                ...
                EditorGUILayout.PropertyField(base.serializedObject.FindProperty(string.Format("m_LODs.Array.data[{0}].isImposter", this.activeLOD)), new GUIContent("Render as imposter"), new GUILayoutOption[0]);
                if (base.serializedObject.FindProperty(string.Format("m_LODs.Array.data[{0}].isImposter", this.activeLOD)).boolValue)
                {
                    EditorGUILayout.PropertyField(base.serializedObject.FindProperty(string.Format("m_LODs.Array.data[{0}].renderShadows", this.activeLOD)), new GUIContent("Render shadows"), new GUILayoutOption[0]);
                    EditorGUILayout.PropertyField(base.serializedObject.FindProperty(string.Format("m_LODs.Array.data[{0}].maxImposterResolution", this.activeLOD)), new GUIContent("Max texture resolution"), new GUILayoutOption[0]);
                    EditorGUILayout.PropertyField(base.serializedObject.FindProperty(string.Format("m_LODs.Array.data[{0}].minImposterResolution", this.activeLOD)), new GUIContent("Min texture resolution"), new GUILayoutOption[0]);
                }
        ...
        private void DrawRenderersInfo(float availableWidth)
        {
            ...
            Rect rect2 = GUILayoutUtility.GetRect(avalibleCollumCount * 60, (float)(num3 * 60) + 1, new GUILayoutOption[] //(float)(num3 * 60)
				{
        ...
        private void HandleAddRenderer(Rect position, IEnumerable<Rect> alreadyDrawn, Rect drawArea)
        {
            ...
            switch (type)
            {
                case EventType.Repaint:
                    ...
                    LODGroupGUI.Styles.m_LODRendererAddButton.Draw(new Rect(position.x - 2f, position.y, position.width, position.height), "  Drag&Drop", false, false, false, false);
                    return;
        ...
        private void DrawRendererButton(Rect position, int rendererIndex)
        {
            ...
            OriginalGOController originalGOController = serializedProperty2.objectReferenceValue as OriginalGOController;
        ...
        private IEnumerable<OriginalGOController> GetRenderers(IEnumerable<GameObject> selectedGameObjects, bool searchChildren)
        {
        ...
        private void AddGameObjectRenderers(IEnumerable<OriginalGOController> toAdd, bool add)
        {
            ...
            foreach (OriginalGOController current in toAdd)
            {
                if (!list.Contains(current))
                {
                    ...
                    serializedProperty.GetArrayElementAtIndex(serializedProperty.arraySize - 1).objectReferenceValue = current;
                    list.Add(current);
                }
            }
            base.serializedObject.ApplyModifiedProperties();
        ...
        private static void UpdateCamera(float desiredPercentage, ImposterController imposterController)
        {
            ...
            float num = ImposterLODUtility.CalculateDistance(SceneView.lastActiveSceneView.camera, relativeScreenHeight, imposterController);
        ...
        private void DrawLODLevelSlider(Rect sliderPosition, List<LODGroupGUI.LODInfo> lods)
        {
            ...
            switch (typeForControl)
            {
                case EventType.MouseDown:
                    {
                        if (current.button == 1 && sliderPosition.Contains(current.mousePosition))
                        {
                            ...
                            {
                                genericMenu.AddItem(new GUIContent("Insert Before"), false, new GenericMenu.MenuFunction(new ImposterControllerEditor.LODAction(lods, cameraPercent, current.mousePosition, this.m_LODs, null).InsertLOD));
                            }
                            bool flag = true;
                            ...
                            {
                                genericMenu.AddItem(new GUIContent("Delete"), false, new GenericMenu.MenuFunction(new ImposterControllerEditor.LODAction(lods, cameraPercent, current.mousePosition, this.m_LODs, DeletedLOD).DeleteLOD));//new LODGroupEditor.LODAction.Callback(this, DeletedLOD)).DeleteLOD));
                            }
                ...
                case EventType.MouseDrag:
                    if (GUIUtility.hotControl == controlID && this.m_SelectedLODSlider >= 0 && lods[this.m_SelectedLODSlider] != null)
                    {
                        ...
                        SerializedProperty serializedProperty = base.serializedObject.FindProperty(string.Format("m_LODs.Array.data[{0}].screenRelativeTransitionHeight", lods[this.m_SelectedLODSlider].LODLevel));
                            ...
                            if (DragAndDrop.objectReferences.Count<UnityEngine.Object>() > 0)
                            {
                                ...
                                if (current.type == EventType.DragPerform)
                                {
                                    ...
                                    if (num == -1)
                                    {
                                        ...
                                        SerializedProperty serializedProperty2 = base.serializedObject.FindProperty(string.Format("m_LODs.Array.data[{0}].screenRelativeTransitionHeight", lods.Count));
                                        if (lods.Count == 0)
                                        {
                                        ...
                                        {
                                            SerializedProperty serializedProperty3 = base.serializedObject.FindProperty(string.Format("m_LODs.Array.data[{0}].screenRelativeTransitionHeight", lods.Count - 1));
            ...
            {
                Camera camera = SceneView.lastActiveSceneView.camera;
                float activeRelativeScreenSize = ImposterLODUtility.RelativeScreenSize(camera, this.m_ImposterController);
                float value = LODGroupGUI.DelinearizeScreenPercentage(activeRelativeScreenSize);
                Vector3 b = this.m_ImposterController.bounds.center;//LODUtility.CalculateWorldReferencePoint(this.m_LODGroup);
